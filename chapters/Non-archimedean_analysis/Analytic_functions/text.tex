...
\subsection{Analytic functions on closed polydiscs}
    ...
    \begin{remark}\label{rmk:restricted_power_series_to_function_ring_is_injective}
        \cref{prop:restricted_power_series_to_function_ring_is_injective} holds for non-archimedean fields of positive characteristic as well.
        The proof uses \Yang{ref.}
    \end{remark}
    ...
    \begin{remark}\label{prop:gauss_norm_does_not_coincides_with_the_suprum_norm_over_non-algebraically_closed_fields}
        ...
        If \(\kk\) is locally compact, the gauss norm on the Tate algebra \(\kk\{ \underline{T/r} \} \) may not coincide with the supremum norm.
    ...
    \begin{proof}
        ...
        Hence we have 
        \[ \|b_k\|r^k = \max_{n\geq k+1} \|a_{k+1+n} x^n\|r^k \leq \max_{n \geq k+1}\|a_{k+1+n}\| r^{n+k} \to 0 \quad \text{ as } k \to \infty. \]
        It follows that \(g(T) \in \kk\{T/r\}\).

        For every \(n > N\), we have 
        \[ \|a_N\| > \|a_{n}\|r^{n-N}. \]
        Hence 
        \[ \left\|\sum_{n=N}^{N+m} a_n x^{n-N-1}\right\| = \|a_N\| \]
