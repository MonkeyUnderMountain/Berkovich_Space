...
\subsection{Finite field extensions}
    ...
    %     Let \(\kk\) be a complete field and \(\bfl\) a finite extension of \(\kk\) with degree \(n = [\bfl : \kk]\).
    %     For any \(a \in \bfl\), define
    %     \[
    %         |a|_{\bfl} := |N_{\bfl/\kk}(a)|_{\kk}^{1/n},
    %     \]
    %     where \(N_{\bfl/\kk}(a)\) is the norm of \(a\) from \(\bfl\) to \(\kk\).
    %     More concretely, viewing \(\bfl\) as an \(n\)-dimensional vector space over \(\kk\), let \(m_a: \bfl \to \bfl\) be the \(\kk\)-linear map defined by multiplication by \(a\).
    %     Then \(N_{\bfl/\kk}(a) = \det(m_a)\).
